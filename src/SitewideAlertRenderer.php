...
class SitewideAlertRenderer implements SitewideAlertRendererInterface {
  ...
  public function build(bool $adminAware = TRUE): array {
    ...
    $sitewideAlerts =  \Drupal::entityTypeManager()->getStorage('sitewide_alert')->loadByProperties(['status' => 1]);
    $show_on = [];
    ...
    $build = [];
    if (in_array($path, $show_on)) {
      $build = [
        '#markup' => '<div id="sitewide-alert" data-sitewide-alert></div>',
        '#attached' => [
          'library' => [
            'sitewide_alert/init',
          ],
